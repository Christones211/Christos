...
LastWeekTest <-5       #Last Week of the Test Dataset
FOrecastingLength <-5  #Foreacsting length on the Test
    ...
    {
      {
        ...
        ThesisDataTime<-ts(t[,Allqty],start=c(2016,LastWeek+1),frequency=52)
        ...
        #Output Fit charts
         ...
         Model1<-auto.arima(ts(ThesisDataTime[1:(length(ThesisDataTime)-LastWeekTest)],start=c(2016,31),frequency=52))
         ...
         lines(ThesisDataTime)
          ...
          ForecastStartd1$ID <- seq.int(nrow(ForecastStartd1))+n2-ForecasthAhaid
...
FittedTotalTime<-ts(FittedTotal[,Actual],start=c(2016,LastWeek+1),frequency=52)
...
Model1<-auto.arima(ts(FittedTotalTime[1:(length(FittedTotalTime)-LastWeekTest)],start=c(2016,31),frequency=52))
fcast_no_holdout <- forecast(Model1,h=FOrecastingLength)
